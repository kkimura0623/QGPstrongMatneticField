...
!
  ...
  real(DP) :: thetaB_0, thetaB_1, dthetaB, thetaB, theta
  real(DP) :: mass2_all(10)
  real(DP) :: thetaB_all(10)
  character(len=256) :: mass_indx(10)
  character(len=256) :: thetaB_indx(10)
  real(DP) :: px,py,pz,pT, pT0,pT1,dpT
  integer :: status(2)
  integer :: ipx,ipy,ipz,ipt,NPT,imass2,NMASS2,ithetaB,NTHETA
  integer :: i,j
  integer :: mu,nu,ia,ib
  integer :: iout
  ...
  !======================================
  !degree between magnetic feild and pT
  !======================================
  NTHETA = 1
  thetaB_all(1) = 0.0_DP
  thetaB_all(2) = PI/6.0_DP
  thetaB_all(3) = PI/4.0_DP
  thetaB_all(4) = PI/3.0_DP
  thetaB_all(5) = PI/2.0_DP
  thetaB_indx(1) = "0"
  thetaB_indx(2) = "30"
  thetaB_indx(3) = "45"
  thetaB_indx(4) = "60"
  ...
  thetaB_0 = 0.0_DP
  thetaB_1 = PI/2.0_DP
  dthetaB = (thetaB_1 - thetaB_0)/NTHETA

#if 0
  do imass2 = 1,NMASS2
  do ithetaB = NTHETA, 5
  mass2 = mass2_all(imass2)
  thetaB = thetaB_all(ithetaB)
  ...
  write(fname,'("epem_xy_",A,".dat")') TRIM(thetaB_indx(ithetaB))
  iout = 111
  ...
  do ipt = 0,NPT
    ...
    !   p1 = (   E, px,   py, pz)
    !   p2 = (   E,-px,   py, pz)
    ! qmon = ( 2*E,  0, 2*py, 2*pz) = p1 + p2
    ! qmon**2 =  4*E**2 - 4*py**2 - 4*pz**2
    !         =  4*(me**2 + px**2 + py**2 + pz**2) - 4*py**2 - 4*pz**2
    !         =  4*me**2 + 4*px**2 + 4*py**2 + 4*pz**2 - 4*py**2 - 4*pz**2
    !         =  4*me**2 + 4*px**2
    !         =  4*(me**2 + px**2) = mass2
    !        => px**2 = (mass2/4 - me**2)
    ...
    py = (pT/2.0_DP)*SIN(thetaB)
    pz = (pT/2.0_DP)*COS(thetaB)
  ...
  write(iout,*)
  close(iout)
  enddo ! end of do ithetaB
  enddo ! end of do imass2

#endif
!  write(fname,'("epem_yz_",A,".dat")')TRIM(thetaB_indx(ithetaB))
!  iout = 111
!  open(iout,file=fname,status='unknown',form='formatted')
!
!  write(*,'("# ",A)')TRIM(_REVISION_)
  ...
  do ipt = 1,NPT
    ...
    p1(1) = 0.0_DP
    p1(2) = (sqrt(py**2+pz**2))*SIN(thetaB - theta)
    p1(3) = (sqrt(py**2+pz**2))*COS(thetaB - theta)
    p1(0) = sqrt( MASS**2 + p1(1)**2 + p1(2)**2 + p1(3)**2 )

    p2(1) = 0.0_DP
    p2(2) = sqrt(py**2+pz**2)*SIN(thetaB + theta)
    p2(3) = sqrt(py**2+pz**2)*COS(thetaB + theta)
    p2(0) = sqrt( MASS**2 + p2(1)**2 + p2(2)**2 + p2(3)**2 )
...
!  dpT = (pT1-pT0)/NPT
  ...
  do ipt = 0,NPT
    ...
    px = SQRT( mass2/4.0_DP - MASS**2 )
!    px = SQRT( mass2/4.0_DP - MASS_MUON**2 )
    py = pT/2.0_DP
