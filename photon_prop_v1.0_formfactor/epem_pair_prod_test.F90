...
!  pol_type = POLTYPE_FUKUSHIMA
  particle_type = PARTICLES_ALL
!  particle_type = PARTICLES_MUON
...
!  fname="epem_xy_300.dat"
  write(fname,'("epem_xy_",A,".dat")')TRIM(mass_indx(imass2))
  iout = 111
...
!    call formfactor01(qmom,eB,r,cn0,cn1,cn2,rn0,rn1,rn2,particle01_type=particle_type,status=status)
    ...
    write(iout,'(9E24.15)') pT, REAL(Rate,kind=KIND(1.0_DP)),REAL(n0),AIMAG(n0),REAL(n1),AIMAG(n1),REAL(n2),AIMAG(n2)
...
!    p2(0) = sqrt( MASS_MUON**2 + p2(1)**2 + p2(2)**2 + p2(3)**2 )
    ...
    write(iout,'(9E24.15)') pT,r, REAL(Rate,kind=KIND(1.0_DP)),REAL(n0),AIMAG(n0),REAL(n1),AIMAG(n1),REAL(n2),AIMAG(n2)
