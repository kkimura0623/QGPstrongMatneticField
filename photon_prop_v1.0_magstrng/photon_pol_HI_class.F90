...
!$OMP PARALLEL DO PRIVATE(komp,n,m,dc,omg0s)
    ...
    do komp=0,NOMP-1
      ...
      if ( n > 2 .and. ABS(n0) < 1000*EPSILON(1.0_DP) .and. ABS(REAL(2*o0(komp),kind=QP)) < 1000*EPSILON(1.0_DP) ) then
      ...
      if ( n > 2 .and. ABS(REAL(2*o0(komp),kind=QP)/REAL(n0,kind=QP)) < 10000*EPSILON(1.0_DP) ) then
        if (m_debug) write(*,'("n0 end at n=",I9," n0=",2ES24.15," dn0=",2ES24.15)')n,n0,2*o0(komp)
        exit loop_n
      endif
      if ( n == m_nmax ) then
