...
!
  ...
  integer :: status(2)
  integer :: ipx,ipy,ipz,ipt,NPT,imass2,NMASS2,ieB
  integer :: i,j,flag
  integer :: mu,nu,ia,ib
  integer :: iout
...
do imass2 = 1, NMASS2
  ...
  write(fname,'("epem_xy_M",A,"_eB",A,".dat")')TRIM(mass_indx(imass2)),TRIM(eB_indx(ieB))
  iout = 111
...
!  enddo ! end of do imass2
  ...
  write(fname,'("epem_yz_M",A,"_eB",A,".dat")')TRIM(mass_indx(imass2)),TRIM(eB_indx(ieB))
  iout = 111
  open(iout,file=fname,status='unknown',form='formatted')
!  write(iout,'("# x-y plane ")')
!  write(iout,'("# mass2[MeV^2]       pT[MeV]         rate ")')
