module intde2_mod
!
! Module for DE-Quadrature 
!
! Original DE program "intde2.f" is written by
! Takuya Ooura,
!  Numerical Integration (Quadrature) - DE Formula (Almighty Quadrature) 
! URL: http://www.kurims.kyoto-u.ac.jp/~ooura/intde.html
!
! 2013/03/15 Fortran90 moduled with function argument void pointer by K.-I. Ishikawa
!
!
!
!---------------------------------------------------------------
!
! Function f(x) to be integrated should be defined as
!  interface
!    real(8) function f(x, p_paras)
!      real(8),          intent(in) :: x
!      character(len=1), intent(in) :: p_paras(:)
!    end function
!  end interface
!
! p_paras(:) contains function arguments with TRANSFER'd.
!
! integration subroutines are modified to 
!         call intde(f,  p_paras, a, b, aw, i, err)
!         call intdei(f, p_paras, a, aw, i, err)
!         call intdeo(f, p_paras, a, omega, aw, i, err)
!
! where p_paras is the function argument list data TRANSFER'd.
!
!
  implicit none
  public
...
subroutine intde(f, paras, a, b, aw, i, err)
  implicit none
  character(len=1), intent(in) :: paras(:)
  interface
    real(8) function f(x, paras)
      real(8), intent(in) :: x
      character(len=1), intent(in) :: paras(:)
    end function
  end interface
  real(8) :: a, b, aw(0 : *), i, err
  integer :: noff, lenawm, nk, k, j, jtmp, jm, m, klim
  real(8) :: epsh, ba, ir, xa, fa, fb, errt, errh, errd, h, iback, irback
...
subroutine intdeiini(lenaw, tiny, eps, aw)
  ...
  real(8) :: efs, hoff
  integer :: noff, nk, k, j
  real(8) :: pi4, tinyln, epsln, h0, ehp, ehm, h, t, ep, em, xp, xm, wp, wm
...
subroutine intdei(f, paras, a, aw, i, err)
  implicit none
  character(len=1), intent(in) :: paras(:)
  interface
    real(8) function f(x, paras)
...
subroutine intdeoini(lenaw, tiny, eps, aw)
  ...
  real(8) :: pi4, tinyln, epsln, frq4, per2, pp, pq, ehp, &
 &           ehm, h, t, ep, em, tk, xw, wg, xa
...
subroutine intdeo(f, paras, a, omega, aw, i, err)
  implicit none
  character(len=1), intent(in) :: paras(:)
  interface
    real(8) function f(x, paras)
...
30            continue
          ...
          do while (abs(fm) .gt. err .and. j .lt. k)
              j = j + 3
              fm = f(a + per * aw(j), paras)
              ir = ir + fm * aw(j + 1)
              fm = fm * aw(j + 2)
              i = i + fm
          end do
          fm = f(a + perw * tk, paras)
          s2 = w02 * fm
          i = i + s2
