...
!
  ...
  call set_limit_landau_level(mmax=4000)
  ...
  do ipt = 0,NPT
    ...
    rate = ProductionRate(p1,p2,eB)

    write(*,'("#  Rate =",2E16.7)') rate

    write(iout,'(3E24.15)') mass2,  pT, rate
...
function ProductionRate(p1,p2,eB) result(rate)
  real(DP), intent(in) :: p1(0:NDIM-1),p2(0:NDIM-1)
...
!  pol_type = POLTYPE_ITAKURA_FULL
  pol_type = POLTYPE_FUKUSHIMA    ! Lowest Landau Level approximation
  ...
  do ib=0,NDIM-1
    do nu=0,NDIM-1
    do mu=0,NDIM-1
      ctmp = ctmp + Lep(mu,nu)*Prop(mu,ia)*CONJG(Prop(nu,ib))*Src(ia,ib)
    enddo
    enddo
  enddo
  enddo
